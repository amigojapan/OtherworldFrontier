...
local function alertBoxYesClickedComplete( )
    ...
    composer.gotoScene( "humanStoreHelingPotions", options )
end
...
function verifyPurchaseEN(userinput)
    ...
    if not isInteger(userinput) then
        RESETQUE()
        QUESLOWPRINT("^^Sorry, the number of KG of food must be a number...^")
        ...
        if price>composer.getVariable( "gold") then
            ...
            AlertBox(
            "",--no title
            "You want to buy:"..userinput..", KG of food right?",
...
function shopAriveEN()
    ...
    QUESLOWPRINT("^^Food supplies cost 10 grams of gold ^")
    QUESLOWPRINT("per kilogram, how many kilograms of ^")
    QUESLOWPRINT("food do you want to buy?^")
