...
function goHuntingPaczel()
    print("3Before setting: x:", caravanGroup.x, "y:", caravanGroup.y)
    saveCaravan()
    print("aftersavecaravan in go huntin")
    composer.setVariable("numberOfPowerUps",5)
    composer.setVariable("numberOfMonsters",5)

    if landmarkShown or composer.setVariable("wentHunting")  then
        return 
     end
     landmarkShown=true
     setGamePausedState(true)
...
function scene:show(event)
    ...
    elseif (phase == "did") then
        ...
        if not composer.getVariable("justStartedGame") then
            composer.setVariable("justStartedGame", true)
        ...
        if composer.getVariable( "language" ) == "English" then
            ...
            showTextArea()
            CLS()
            if not composer.getVariable("justStartedGame2") then
                composer.setVariable("justStartedGame2", true)
                gameStartEN()
            end
        elseif composer.getVariable( "language" ) == "Japanese" then
            setGamePausedState(true)
...
end end--**added these two end statements cause they were missing, but now after hunt the every sprite is doubled and returned to its initial state.
...
--(done in town, I dont think it is nesseary in the store now)add the ammount of the item we have inside the store and in the menu
--(fixed)new bug, name of character appears in shopping screen, I think I can fix it by changing the value of the defaultname thing right when the game starts
...
--(done)dont allow events to happen if continue button is visible, this should prevent the bus of the troll attackign and the continue button nto dismissing the scrren(I think)
...
--(I cant find the cuase of this in the code)bug, it seems it did not display when amigojapan was uncursed but he got uncursed anyway
